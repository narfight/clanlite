...
$sql = "SELECT a.*, section.nom FROM `".$config['prefix']."match` AS a LEFT JOIN `".$config['prefix']."section` AS section ON section.id='".$session_cl['section']."' WHERE (a.section ='".$session_cl['section']."' OR a.section ='0' OR section.limite ='0') AND a.date > '".(time()-60*60*2) ."' ORDER BY a.date ASC";
