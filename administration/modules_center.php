...
if (!empty($_POST['envoyer']))
{
	...
	$sql = "INSERT INTO `".$config['prefix']."modules` (nom, ordre, place, call_page, etat) VALUES ('".$_POST['nom']."', '".$_POST['num']."', 'centre', '".$_POST['module']."', '".$_POST['activation']."')";
	...
	require($root_path.'modules/'.$_POST['module']);
	redirec_text('modules_center.php', $langue['redirection_module_add'], 'admin');
}
if (!empty($_POST['envois_edit']))
{
	$_POST = pure_var($_POST);
	$sql = "UPDATE `".$config['prefix']."modules` SET nom='".$_POST['nom']."', ordre='".$_POST['num']."', etat='".(($_POST['activation'] != 0)? 1 : 0)."' WHERE id='".$_POST['for']."'";
	if (!$rsql->requete_sql($sql))
	{
		sql_error($sql, $rsql->error, __LINE__, __FILE__);
	}
...
if (!empty($_POST['Supprimer']))
{
	...
	redirec_text('modules_center.php', $langue['redirection_module_dell'], 'admin');
}
...
$template->assign_block_vars('header_posix_center' , '');
$template->assign_vars(array(
	'ICI' => session_in_url('modules_center.php'),
...
$sql = "SELECT id,nom,ordre,call_page,etat,place FROM ".$config['prefix']."modules WHERE place='centre' ORDER BY `ordre` ASC";
if (! ($get = $rsql->requete_sql($sql)) )
{
	sql_error($sql, $rsql->error, __LINE__, __FILE__);
}
...
{
	$template->assign_block_vars('header_posix_center.liste_centre', array(
		'ID' => $liste['id'],
		'NOM' => $liste['nom'],
		'NUM' => $liste['ordre'],
