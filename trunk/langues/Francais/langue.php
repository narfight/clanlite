<?php
/****************************************************************************
 *	Fichier		: langue.php												*
 *	Copyright	: (C) 2006 ClanLite											*
 *	Email		: support@clanlite.org										*
 *																			*
 *   This program is free software; you can redistribute it and/or modify	*
 *   it under the terms of the GNU General Public License as published by	*
 *   the Free Software Foundation; either version 2 of the License, or		*
 *   (at your option) any later version.									*
 ***************************************************************************/
/*
si vous voulez traduire ce fichier dans un autre langue :
seul le texte après le = entre les '' doit être traduit, si dans votre traduction vous utilisez un caractère ", vous devez le faire suivre d'un \
Parfois vous verrez %f, %s, %d, %1\$s, %2\$s, %3\$s, ce sont des repères qui désignent au site de remplacer par une information.
voici un exemple de traduction : $langue['test_traduction'] = "je suis \"malade\" car %1\$s m'a frappé sur la ù2\$s";
*/
$langue['boutton_liste_game'] = "Serveurs de jeux";
$langue['titre_menu_pub'] = "Le menu";
$langue['boutton_news'] = "Les news";
$langue['boutton_forum'] = "Forum";
$langue['boutton_inscription'] = "Inscription";
$langue['boutton_match'] = "Les matchs";
$langue['boutton_calendrier'] = "Calendrier";
$langue['boutton_reglement'] = "Réglement";
$langue['boutton_result_match'] = "Résultats";
$langue['boutton_telecharger'] = "Télécharger";
$langue['boutton_entrainements'] = "Entraînements";
$langue['boutton_connect'] = "Partie privée";
$langue['boutton_deconnect'] = "Déconnecter";
$langue['boutton_liens'] = "Liens";
$langue['boutton_org_rencontre'] = "Proposer une rencontre";
$langue['boutton_liste_membres_groupe'] = "Les membres";
$langue['bien_venu_site'] = "Bienvenue sur le site des %1\$s.";

$langue['error_sql_titre'] = "Erreur de requête SQL.";
$langue['error_sql_requette'] = "Requête";
$langue['error_sql_erreur'] = "Erreur reçue";
$langue['error_sql_endroit'] = "Localisation";
$langue['error_sql_endroit_2'] = "Ligne %1\$u dans %2\$s";

$langue['recrute_s'] = "Nous cherchons [b]un[/b] nouveau joueur pour notre clan.";
$langue['recrute_p'] = "Nous cherchons [b]%d[/b] nouveaux joueurs pour notre clan.";
$langue['recrute'] = "Nous sommes à la recherche de nouveaux joueurs dans notre clan.";
$langue['recrute_pas'] = "Nous ne sommes pas à la recherche de nouveaux joueurs.";

$langue['exec_time_rsql'] = "Page exécutée en %fs || %d requêtes sql.";
$langue['copyright'] = "Créé par <a href=\"mailto:narfight@clanlite.org\">Narfight</a>, <a href=\"http://www.clanlite.org\" onclick=\"window.open('http://www.clanlite.org');return false;\">ClanLite</a> V%1\$s © 2000-2005";
$langue['deconect_login'] = "Votre session est fermée.<br />Vous ne serez plus automatiquement reconnu par le site à présent.";

$langue['where_unknow'] = "Vient de se connecter au site";
$langue['where_news'] = "Regarde les nouvelles du front";
$langue['alert_affiche'] = "Les alertes du jour";
$langue['news_titre'] = "Quoi de neuf&nbsp;?";
$langue['poste_le'] = "Posté le";
$langue['poste_par'] = "par";
$langue['0_reaction'] = "Il n'y a pas de réactions.";
$langue['1_reaction'] = "Il y a <b>une</b> réaction.";
$langue['plus_reaction'] = "Il y a eu %s réactions.";
$langue['reaction_add'] = "Votre réaction a été ajoutée.";
$langue['reaction_dell'] = "La réaction est supprimée.";
$langue['reaction_login'] = "Cliquez ici si vous êtes membre du clan.";
$langue['erreur_news_no_found'] = "La nouvelle n'a pas été trouvée.";
$langue['reaction_de'] = "Réaction de %1\$s";

$langue['frame_erreur_id'] = "L'id du site à afficher dans une Iframe est introuvable.";
$langue['where_rss'] = "Utilise le système de nouvelles RSS.";
$langue['where_xml_mp3'] = "Vient de lancer le lecteur MP3";

$langue['titre_liens'] = "Nos liens";
$langue['where_liens'] = "Regarde les liens";
$langue['liens_repertoire_choix_titre'] = "Répertoire de liens";
$langue['liens_repertoire_choix'] = "Choisissez un répertoire de liens.";

$langue['parpage_suivant'] = "Suivant";
$langue['parpage_precedent'] = "Précédent";

$langue['titre_entrainement'] = "Les entraînements";
$langue['where_entrainement'] = "Regarde les entraînements";
$langue['message_cote_prive'] = "Information Privée";

$langue['titre_calendrier'] = "Calendrier";
$langue['where_calendrier'] = "Regarde le calendrier";
$langue['calendrier_entrai'] = "Entrainement";
$langue['calendrier_match'] = "Match contre les %s";
$langue['calendrier_annif'] = "Anniversaire de <a href=\"%1\$sservice/profil.php?link=%2\$u\">%3\$s</a>";
$langue['calendrier_annif_date'] = "Anniversaire d'entrée de <a href=\"%1\$sservice/profil.php?link=%2\$u\">%3\$s</a>";

$langue['titre_connecte'] = "Les connectés";
$langue['where_connecte'] = "Regarde les connectés";
$langue['no_profil'] = "Pas de profil";
$langue['guest'] = "Visiteur";

$langue['where_iframe'] = "regarde un lien depuis une Iframe dans le site";

$langue['titre_defier'] = "Proposer un défi";
$langue['where_defier'] = "propose un match";
$langue['nom_clan_opose'] = "Le nom de votre clan";
$langue['url_clan_opose'] = "URL de votre clan";
$langue['date_defit'] = "Date du match";
$langue['heure_defit'] = "L'heure du match";
$langue['nombre_joueurs'] = "Le nombre de joueurs par équipe";
$langue['info_en_plus'] = "Des informations en plus";
$langue['user_envois_defit'] = "Votre demande est enregistrée, la personne chargée des matchs recevra vos informations";
$langue['mail_titre_defit_prop'] = "Une demande de rencontre avec les %s";
$langue['mail_defit_prop'] = "Un clan a demandé une recontre avec notre clan,\nallez sur le site pour plus d'info.";

$langue['titre_download'] = "Les groupes de téléchargement";
$langue['where_download'] = "sélectionne un téléchargement";
$langue['download_top_10_def'] = "Les 10 fichiers les plus téléchargés";
$langue['download_top_10'] = "Le TOP 10 des téléchargements";
$langue['download_list'] = "La liste des fichiers";
$langue['download_modif'] = "Dernière modification";
$langue['download_bt_cote'] = "Vote";
$langue['download_bt_telecharger'] = "Télécharger";
$langue['download_no_vote'] = "Pas de vote";
$langue['download_nombre'] = "Télécharger";
$langue['download_vote_titre'] = "Les groupes de téléchargement";
$langue['download_vote_explication'] = "Vous allez voter pour un fichier";
$langue['download_vote_envoyer'] = "Voter";
$langue['user_envois_vote'] = "Votre vote a été pris en considération";
$langue['download_alt_psw'] = "Section demandant un code pour le téléchargement des fichiers";
$langue['download_psw_erreur'] = 'Vous n\'avez pas rentré le bon code pour télécharger ce fichier';
$langue['download_psw_demande'] = 'Code de téléchargement';

$langue['def_equipe'] = "Description des équipes";

$langue['where_membre_group'] = "Regarde les membres par groupe";
$langue['where_membre_liste'] = "Regarde la liste des membres";
$langue['membre_dell'] = "Le membre a été supprimé";
$langue['titre_liste_membres'] = "La liste des membres";
$langue['no_section'] = "Sans section détérminée";
$langue['no_equipe'] = "Dans cette section mais sans équipe déterminée";

$langue['titre_match'] = "Les matchs";
$langue['where_match'] = "Regarde les matchs à venir dans la partie publique";
$langue['where_match_prive'] = "Regarde les matchs à venir dans la partie privée";
$langue['contre_qui'] = "Contre les";
$langue['nbr_joueurs'] = "Joueurs par clan";
$langue['heure_chat'] = "Sur le chat à" ;
$langue['quelle_section'] = "Pour la section";
$langue['team_ok'] = "La team pour le match";
$langue['team_reserve'] = "La team de réserve";
$langue['team_demande'] = "Les demandes";
$langue['ajouter/supprimer_demande_match'] = "Demander/Annuler sa participation au match";
$langue['match_inscrit'] = "Match où vous étes inscrit";
$langue['match_pas_inscrit'] = "Match où vous n'étes pas inscrit";

$langue['toutes_section'] = "Sans limite";
$langue['user_envois_demande_ok'] = "Votre demande de participation au match est enregistrée";
$langue['user_envois_annule_demande'] = "Votre demande de participation au match est supprimée";

$langue['titre_profil'] = "Profil de %s";
$langue['where_profil'] = "Regarde un profil";
$langue['alt_profil_image'] = "Image du membre";
$langue['nom'] = "Nom";
$langue['prenom'] = "Prénom";
$langue['login'] = "Pseudo";
$langue['sex'] = "Sexe";
$langue['age'] = "Âge";
$langue['user_date'] = "Date d'entrée dans le clan";
$langue['site_web'] = "Site Web";
$langue['e-mail'] = "E-mail";
$langue['favori_arme'] = "Arme favorite";
$langue['alt_arme'] = "Arme favorite";
$langue['last_connection'] = "Dernière connection";
$langue['cri_guerre'] = "Cri de guerre";
$langue['roles'] = "Rôle dans le clan";
$langue['histoire'] = "Petite histoire";
$langue['alt_medaille'] = "Médaille offerte à ce membre";
$langue['erreur_profil_no_found'] = "Le profil que vous voulez voir/modifier n'existe pas";

$langue['titre_match_rapport'] = "Les résultats des matchs";
$langue['where_match_rapport'] = "Regarde les matchs joués";
$langue['titre_liste_match_rapport'] = "Joué le %1\$s contre les %2\$s";
$langue['score_nous'] = "Score de la team";
$langue['score_eux'] = "Leur score";
$langue['resume_match_gagnee'] = "Nombre de matchs gagnés";
$langue['resume_match_perdu'] = "Nombre de matchs perdus";
$langue['resume_match_norm'] = "Nombre d'égalités";
$langue['no_match_joue'] = "Le clan n'a pas encore fait de match";
$langue['no_futur_match'] = "Pas de match prévu";

$langue['titre_reaction'] = "Les réactions";
$langue['where_reaction'] = "Regarde les réactions d'une news";
$langue['erreur_reaction_vide'] = "Vous n'avez pas saisi votre réaction.";
$langue['erreur_no_membre'] = "Si vous postez sous le nom d'un membre, vous devez taper son code.<br />";
$langue['add_commentaire'] = "Ajouter un commentaire";
$langue['dell_reaction'] = "Supprimer cette réaction";

$langue['titre_reglement'] = "Le réglement du clan";
$langue['where_reglement'] = "Lit le réglement du clan";
$langue['no_reglement'] = "Il n'y a pas de réglement pour le moment";

$langue['titre_serveur_jeux'] = "Le serveur de jeux des %s";
$langue['titre_serveurs_jeux'] = "Les serveurs de jeux des %s";
$langue['titre_serveur_jeux_autre'] = "Les serveurs des autres clans";
$langue['serveur_jeux_down'] = "Le serveur de jeux n'a pas répondu";
$langue['where_serveur_jeux'] = "Regarde la page sur le serveur de jeux du clan";
$langue['nom_serveur_jeux'] = "Nom du serveur";
$langue['password_serveur_jeux'] = "Présence d'un code";
$langue['version_serveur_jeux'] = "Version du serveur";
$langue['gametype_serveur_jeux'] = "Type de partie";
$langue['ping_min_max_serveur_jeux'] = "Votre PING doit être dans une fourchette de";
$langue['nbr_place_serveur_jeux'] = "Joueur";
$langue['map_serveur_jeux'] = "Map actuelle";
$langue['alt_img_map_serveur_jeux'] = "Aperçu de la map %s";
$langue['rotation_map_serveur_jeux'] = "Rotation des maps";
$langue['next_map_serveur_jeux'] = "Prochaine map";
$langue['obj_axis_serveur_jeux'] = "Objectis Axis";
$langue['obj_allier_serveur_jeux'] = "Objectis alliés";
$langue['liste_joueur_serveur_jeux'] = "Joueurs connectés";
$langue['name_serveur_jeux'] = "Nom";
$langue['score_serveur_jeux'] = "Score";
$langue['ping_serveur_jeux'] = "Ping";
$langue['enemy_serveur_jeux'] = "Méchant";
$langue['kia_serveur_kia'] = "KIA";
$langue['frags_serveur_jeux'] = "Frags";

$langue['titre_serveur_list'] = "Liste de serveurs de jeux";
$langue['where_serveur_list'] = "Regarde la liste des serveurs de jeux";

$langue['titre_lost_code'] = "Retrouver son code de connection";
$langue['where_lost_code'] = "Retrouver son code de connection";
$langue['def_code_activation'] = "Si vous avez reçu le code d'activation, vous pouvez le taper ici";
$langue['user_envois_newscodeok'] = "Votre nouveau code est validé, vous pouvez vous en servir pour vous connecter.";
$langue['user_envois_newscodeerror'] = "Le code que vous avez saisi n'est pas valide.";
$langue['user_envois_mailnofound'] = "Votre e-mail n'a pas été trouvé.";
$langue['user_envois_mailinvalide'] = "Vous n'avez pas entré un e-mail valide.";
$langue['user_envois_mailinvalideenvoye'] = "Un e-mail vous a été envoyé avec un nouveau code et une clé pour l'activer.";
$langue['titre_mail_codeperdu'] = "Votre nouveau code pour le site des %s";
$langue['corps_mail_codeperdu'] = "Bonjour %1\$s,\nVoici votre login et votre nouveau code :\nVotre code: %2\$s\nVotre login: %3\$s\nPour activer ce nouveau code, vous devez vous diriger sur cette URL : %4\$suser/code-perdu.php?activ_pw=%5\$s\n_________________\npour toute question : ".$config['master_mail'];

$langue['titre_admin_edit_user'] = "Gestion administrative de %1\$s";
$langue['where_admin_edit_user'] = "Modifie le compte d'un membre";
$langue['where_edit_user'] = "Modifie son profil";
$langue['titre_edit_user'] = "Modification de votre compte";
$langue['user_envois_edit_profil'] = "Le profil du membre a été mis à jour";

$langue['titre_entree_user'] = "Les matchs et entraînements à ne pas rater";
$langue['titre_entree_admin'] = "Informations pour les administrateurs";
$langue['where_entree_user'] = "Est sur l'accueil de la partie privée";
$langue['admin_divers_nfo'] = "Quelques informations";
$langue['admin_user_nfo'] = "Avertissement au sujet des membres";
$langue['admin_nombre_membre'] = "Nombre de membres dans le clan";
$langue['admin_nombre_match'] = "Nombre de matchs prévus";
$langue['admin_nombre_old_match'] = "Nombre de vieux matchs qui attendent leurs rapport";
$langue['admin_nombre_demande_match'] = "Nombre de demandes de matchs en attente";
$langue['user_verif'] = "Vérifier !";
$langue['user_ok'] = "Ok";
$langue['user_match_place'] = "Il y a encore de la place pour les matchs suivants";
$langue['user_entrainement_place'] = "Le prochain entraînement";
$langue['info_match_place'] = "Contre les %1\$s qui se déroulera le %2\$s à %3\$s";
$langue['heure_entrai'] = "se déroulera à";
$langue['date_entrai'] = "Prévu le";
$langue['info_entrai'] = "Information publique";
$langue['info_prive_entrai'] = "Information privée";

$langue['admin_news_cl_titre'] = "Mise à jour de ClanLite";
$langue['admin_news_cl_on'] = "Une nouvelle version de Clanlite est disponible<br />(votre version: %1\$s, version disponible: %2\$s)";
$langue['admin_news_cl_off'] = "Recherche désactivée";
$langue['admin_news_cl_toggle'] = "Activer/Désactiver la détection de nouvelle mise à jour";

$langue['titre_lire_news_nfo'] = "Lisez bien ceci";
$langue['where_lire_news_nfo'] = "Lit le texte après l'inscription";

$langue['text_bienvenu_vide'] = "Votre inscription a bien été enregistrée, contactez l'administrateur pour activer votre compte";
$langue['titre_inscription'] = "Formulaire d'inscription au clan";
$langue['where_inscription'] = "Remplit le formulaire d'inscription";
$langue['inscription_fermée'] = "Les inscriptions sont bloquées, vous ne pouvez pas envoyer ce formulaire.";
$langue['user_envois_inscription'] = "Votre inscription est bien enregistrée";

$langue['titre_login'] = "Entrez dans la partie privée";
$langue['where_login'] = "Saisit son code/login";
$langue['save_code_login'] = "Se souvenir de moi";
$langue['lost_psw'] = "vous avez perdu votre code ?";
$langue['non_active'] = "Votre compte n'a pas été validé par un administrateur, contactez-le pour plus d'informations";
$langue['code_login_incorrect'] = "Votre code ou votre login est incorrect, attention aux capitales !";
$langue['erreur_secu'] = "Vous essayez d'entrer dans une partie Privée qui peut nécessiter certains pouvoirs ; il est aussi possibles que votre navigateur Web ait bloqué les cookies";

$langue['redirection_txt_vide'] = "Les modifications ont été exécutées.<br />Vous allez être redirigés dans 3 secondes, mais vous pouvez <a href=\"%1\$s\">forcer le chargement de la page</a>.";
$langue['redirection_txt_nonvide'] = "%1\$s<br />Cliquez <a href=\"%2\$s\">ici</a> si vous n'êtes pas redirigé après 3s";

// Les modules officiel
$langue['where_module_central'] = "Regarde un module central";
$langue['module_compteur'] = "Il y a eu %1\$d visiteurs et il y a %2\$d connecté(s)";

// gallery
$langue['where_module_gallery'] = "Administre la galerie";
$langue['titre_module_gallery'] = "Administration de la galerie";
$langue['titre_gestion_module_img'] = "Gestion des images";
$langue['titre_liste_module_img'] = "Liste des images";
$langue['titre_configuration_module_img'] = "Configuration des options";
$langue['unit_byte'] = "Octets";
$langue['gallery'] = "Nos images";
$langue['gallery_apercu'] = "Aperçu";
$langue['gallery_config-show'] = "Afficher la configuration";
$langue['gallery_remove'] = "supprimer";
$langue['gallery_txt_url'] = "Emplacement de l'image";
$langue['gallery_size_img'] = "Taille de l'image";
$langue['gallery_size_tumb'] = "Taille des miniatures";
$langue['gallery_max_size'] = "Poids maximum des images";
$langue['redirection_gallery_edit'] = "Les dimensions ont été mis à jour";
$langue['redirection_gallery_dell'] = "L'image a été supprimée";
$langue['the_file_has_been_uploaded'] = "Le fichier %s a été téléchargé";
$langue['upload_error'] = "Il y a eu une erreur lors du téléchargement du fichier, essayer encore !";
$langue['not_supported'] = "Ce format n'est pas supporté";
$langue['mini_error'] = "Il y a eu une erreur lors de la création de la miniature!";
$langue['com_img'] = "Commentaire";

$langue['where_module_show-section'] = "Gère le module pour afficher une section";
$langue['titre_module_show-section'] = "Afficher une section [Module]";
$langue['redirection_module_show-section'] = "Module mis à jour";

$langue['ts_connection_module_liens'] = "Cliquez ici pour rejoindre le serveur vocal";
$langue['ts_connection_module_titre'] = "Connection au serveur vocal";

$langue['where_module_aléatoire'] = "Gère le module aléatoire";
$langue['titre_module_aléatoire'] = "Les aléatoire [Module]";
$langue['titre_gestion_module_aléatoire'] = "Gestion des aléatoires";
$langue['titre_liste_module_aléatoire'] = "Liste des aléatoires";
$langue['redirection_module_aléatoire_add'] = "L'objet aléatoire est ajouté";
$langue['redirection_module_aléatoire_edit'] = "L'objet aléatoire est mis à jour";
$langue['redirection_module_aléatoire_dell'] = "L'objet aléatoire est supprimé";

$langue['where_module_partenaire'] = "Gère les partenaires du clan";
$langue['titre_module_partenaire'] = "Les partenaires [Module]";
$langue['titre_gestion_module_partenaire'] = "Gestion des partenaires";
$langue['titre_liste_module_partenaire'] = "Liste des partenaires";
$langue['redirection_module_partenaire_add'] = "Le partenaire à ajouter";
$langue['redirection_module_partenaire_edit'] = "Le partenaire a été mis à jour";
$langue['redirection_module_partenaire_dell'] = "Le partenaire a été supprimé";
$langue['module_partenaire_nom'] = "Nom du site";

$langue['where_module_shoutbox'] = "Gère la shoutbox";
$langue['titre_module_shoutbox'] = "La shoutbox [Module]";
$langue['titre_module_shoutbox_del'] = "Suppresion des messages indésirable";
$langue['redirection_module_shoutbox_edit'] = "La shoutbox a été mis à jour";
$langue['redirection_module_shoutbox_dell'] = "Message(s) supprimé";
$langue['module_shoutbox_nbr_lignes'] = 'Nombre de lignes dans le module';
$langue['module_shoutbox_vider'] = 'Vider les messages';

$langue['where_module_module_custom'] = "Modifie un module perso";
$langue['titre_module_module_custom'] = "Module perso [Module]";
$langue['titre_module_module_custom_c'] = "Module perso [Module Central]";
$langue['module_custom_contenu'] = "Contenu du module";
$langue['module_custom_titre'] = "Titre du module";
$langue['redirection_module_custom_edit'] = "Le module perso a été mis à jour";

$langue['where_module_connect_ts'] = "Gère la connection au Teamspeak";
$langue['titre_module_connect_ts'] = "Connection Teamspeak [Module]";
$langue['redirection_module_connect_ts_edit'] = "L'adresse du serveur Teamspeak a été mis à jour";
$langue['redirection_module_connect_ts_psw_channel'] = "Code de la chambre";
$langue['redirection_module_connect_ts_channel'] = "La chambre";
$langue['redirection_module_connect_ts_connection'] = "Connection";

$langue['where_module_match'] = "Gére le module pour le prochain match";
$langue['titre_module_match'] = "Prochain match [Module]";
$langue['redirection_module_match_edit'] = "Le module match à été mis à jour";
$langue['match_module_show_map'] = "Afficher les images des maps";

$langue['module_last_match_affiche_info'] = "Voir/cacher le résumé";

$langue['where_module_serveur_game'] = "Gère le module serveur de jeux";
$langue['titre_module_serveur_game'] = "Serveur de jeux [Module]";
$langue['clanserveur_module_serveur_game'] = "Serveur du clan";
$langue['image_map_module_serveur_game'] = "Afficher la map qui tourne sur le serveur";
$langue['liste_module_serveur_game'] = "Afficher la liste des joueurs";
$langue['redirection_module_serveur_game_edit'] = "Le serveur de jeux à afficher a été changé";

$langue['titre_newsletter'] = "Newsletter";
$langue['where_newsletter'] = "Ajoute son e-mail à une newsletter";
$langue['newsletter_send_ok'] = "Vous avez bien été ajouté à la liste de diffusion de nos nouveautés.";
$langue['newsletter_doublon'] = "Vous êtes dèjà inscrit sur cette liste de diffusion.";
$langue['newsletter_deinscription'] = "Désinscription";
$langue['newsletter_deinscription_ok'] = "Votre e-mail a été supprimé de la liste";
$langue['newsletter_mailiste_send'] = "Pour la Newsletter";

$langue['where_module_webpost'] = "Gère le module de Webpost";
$langue['server_teamspeak_webpost'] = "Serveur Teamspeak qui envoit des infos";
$langue['where_cl_module_webpost'] = "Regarde un Webpost";
$langue['titre_module_webpost'] = "TeamSpeak Webpost [Module]";
$langue['module_webpost_titre_aide'] = "Configurer le serveur TeamSpeak";
$langue['module_webpost_txt_aide'] = "Dans l'administration web de votre serveur TeamSpeak, dirigez-vous sur \"Server settings\" et remplissez le champ \"Server WebPost PostUrl\" comme ceci : %s<br />\nL'affichage sera actualisé toutes les 5 minutes par le serveur s'il est bien configuré.";
$langue['erreur_webpost_bad_port'] = "Le serveur TeamSpeak dit que votre serveur n'existe pas, vérifiez le port de communication.";
$langue['erreur_webpost_no_reply'] = "Le serveur TeamSpeak ne répond pas, vérifiez l'adresse IP.";
$langue['module_webpost_add'] = "Attendez quelques minutes pour que le serveur envoie les informations de base.";

// partie administration de CL
$langue['prive_titre_page'] = "Partie privée du site";
$langue['menu_titre_admin'] = "Administration";
$langue['menu_titre_user'] = "Membres";
$langue['menu_titre_module'] = "Module";
$langue['menu_titre_module_center'] = "Module central";
$langue['menu_titre_go_publique'] = "Partie publique";
$langue['menu_titre_prive_entree'] = "Retour à la page d'entrée de la partie privée";

$langue['titre_alert'] = "Les alertes";
$langue['where_alert'] = "Modifie les alertes";
$langue['titre_alert_gestion'] = "Gestion des alertes";
$langue['titre_alert_list'] = "Liste des alertes";
$langue['redirection_alert_add'] = "L'alerte a été ajoutée";
$langue['redirection_alert_edit'] = "L'alerte a été mise à jour";
$langue['redirection_alert_dell'] = "L'alerte a été supprimée";
$langue['date_dell_alert'] = "Date de suppresion";
$langue['heure_dell_alert'] = "Heure de suppresion";
$langue['opt_auto_dell_alert'] = "Supprimer automatiquement à une date";
$langue['opt_auto_dell_desactiver'] = "Aucune";

$langue['titre_calendrier'] = "Calendrier";
$langue['where_calendrier'] = "Modifie les événement du calendrier";
$langue['titre_calendrier_gestion'] = "Gestion du calendrier";
$langue['titre_calendrier_list'] = "Liste des événement";
$langue['redirection_calendrier_add'] = "L'événement a été ajouté";
$langue['redirection_calendrier_edit'] = "L'événement a été mis à jour";
$langue['redirection_calendrier_dell'] = "L'événement a été supprimé";
$langue['cyclique_calendrier'] = "Répéter l’événement chaque année";
$langue['cyclique_calendrier_sort'] = "Cyclique";

$langue['titre_config_site'] = "Configuration générale";
$langue['config_site_base_titre'] = "Configuration de base";
$langue['config_site_avancée_titre'] = "Configuration avancée";
$langue['config_inscription_titre'] = "Les inscriptions";
$langue['where_config_site'] = "Configure le site";
$langue['redirection_config_ok'] = "La configuration a été mise à jour";
$langue['config_tag_clan'] = "Le TAG du clan";
$langue['config_nom_clan'] = "Le nom du clan";
$langue['config_domain_site'] = "URL du site";
$langue['config_path_site'] = "Répertoire du site";
$langue['config_url_config'] = "URL du forum";
$langue['config_webmasteur'] = "E-mail du webmaster";
$langue['config_tmp_session'] = "Minute de validation de la session";
$langue['config_user_match'] = "Membre chargé des matchs";
$langue['config_show_recrute_index'] = "Afficher l'état de recrutement sur l'index";
$langue['config_etat_recrute'] = "Recrutement";
$langue['config_recrute_limit'] = "Limité";
$langue['config_langue'] = "Langue par default";
$langue['config_skin'] = "Thème par default";
$langue['config_recrutement_limit_txt'] = "Limitation des inscriptions";
$langue['config_nbr_par_page'] = "Nombre de lignes par page";
$langue['config_help_nbr/page'] = "<b>Exemple :</b> X fichiers de t&eacute;l&eacute;chargement par page, ou de rapport de match";
$langue['config_reglement'] = "Réglement du clan";
$langue['config_msg_news_membre'] = "Message de bienvenue apres l'inscription";
$langue['config_mail_send_titre'] = "L'envoi d'e-mail";
$langue['config_send_by'] = "Envoi d'e-mail";
$langue['config_by_smtp'] = "Par SMTP";
$langue['config_by_php'] = "Par mail()";
$langue['config_smtp_server_ip'] = "Serveur SMTP";
$langue['config_smtp_server_port'] = "Port du serveur SMTP";
$langue['config_smtp_code'] = "Mot de passe SMTP";
$langue['config_smtp_login'] = "Nom d'utilisateur SMTP";
$langue['config_scan_game_server'] = "Scan les serveurs de jeux";
$langue['config_scan_game_server_udp'] = "Par UDP";
$langue['config_scan_game_server_http'] = "Par HTTP";
$langue['config_help_scan_game_server'] = "Essayez l\'une ou l\'autre option si vous n\'arrivez pas à afficher les informations sur votre serveur de jeux";
$langue['config_show_grade'] = "Activer la gestion des grades pour les membres du clan";
$langue['config_refresh'] = "Intervalle d'actualisation des données externe (en minutes)";
$langue['config_help_refresh'] = "Plus ce temps est petit, plus le site sera souvent lent car il actualisera les données externes";
$langue['config_time_zone'] = "Fuseau horaire";
$langue['config_heure_ete'] = "Changement de l'heure d'été";

$langue['where_defit_admin'] = "Regarde les demandes des autres clans pour un futur match";
$langue['titre_defit_admin'] = "Demandes de rencontres";
$langue['titre_defit_notif'] = "Réponse à la demande";
$langue['titre_list_defit'] = "Liste des demandes";
$langue['redirection_defit_add'] = "L'e-mail est envoyé, le match a été rajouté et la demande est supprimée, il faut regarder s'il n'y a rien à changer dans la config du match !";
$langue['redirection_defit_dell'] = "La demande a été supprimée";
$langue['reponce_defit_oui'] = "Bonjour membre des %1\$s,\nVous nous avez proposé un match le %2\$s à %3\$s.\nNous nous y présenterons avec nos %4\$s joueurs comme prévu.\nRendez-vous au match :-D\n\nÀ bientôt, %5\$s (%6\$s) chargé des matchs";
$langue['reponce_defit_non'] = "Bonjour membre des %1\$s,\nVous nous avez proposé un match le %2\$s à %3\$s.\nNous ne nous présenterons pas à ce match, nous en sommes désolés\n\nÀ bientôt, %4\$s (%5\$s) chargé des matchs";
$langue['titre_mail_defit'] = "En réponse à votre proposition de défi contre les %1\$s";
$langue['clan_opose'] = "Le clan";
$langue['contact_clan_op'] = "Contacter";
$langue['info_defit_admin'] = "Informations";
$langue['reponce_defit'] = "Répondre";

$langue['where_download_admin'] = "Gère les fichiers en téléchargement";
$langue['titre_download_admin'] = "Les téléchargements";
$langue['titre_gestion_download_admin'] = "Gestion des groupes/fichiers";
$langue['titre_liste_download_admin'] = "Liste des fichiers dans leurs groupes";
$langue['redirection_dl_add'] = "Le fichier à ajouter";
$langue['redirection_dl_edit'] = "Le fichier a été mis à jour";
$langue['redirection_dl_dell'] = "Le fichier a été supprimé";
$langue['redirection_group_dl_add'] = "Le groupe à ajouter";
$langue['redirection_group_dl_edit'] = "Le groupe a été mis à jour";
$langue['redirection_group_dl_dell'] = "Le groupe a été supprimé";
$langue['redirection_group_dl_pasvide'] = "Vous ne pouvez pas supprimer un groupe s'il n'est pas vide";
$langue['toggle_gestion_fichier'] = "Afficher/Cacher la gestion des fichiers";
$langue['toggle_gestion_group'] = "Afficher/Cacher la gestion des groupes";
$langue['dll_url'] = "Emplacement du fichier";
$langue['group_fichier'] = "Dans le groupe";
$langue['group_dl_password'] = "Entrez un code";

$langue['where_medaille'] = "Gère les médailles d'un membre";
$langue['redirection_medaille'] = "Les médailles ont été changées";
$langue['titre_medaille'] = "Gestion des médailles de %1\$s";

$langue['where_entrain'] = "Est dans l'administration des entraînements";
$langue['titre_entrain'] = "Les entraînements";
$langue['titre_entrain_gestion'] = "Gestion des entraînements";
$langue['titre_entrain_list'] = "La liste des entraînements";
$langue['redirection_entrain_add'] = "L'entraînement à ajouter";
$langue['redirection_entrain_edit'] = "L'entraînement a été mis à jour";
$langue['redirection_entrain_dell'] = "L'entraînement a été supprimé";

$langue['where_equipe'] = "Est dans l'administration des équipes";
$langue['titre_equipe'] = "Les équipes";
$langue['titre_equipe_gestion'] = "Gestion des équipes";
$langue['help_equipe'] = "<b>Explication :</b> Les équipes sont un moyen de classer dans les sections les joueurs, vous pouvez faire par exemple une équipe Sniper et une équipe Rusher.";
$langue['titre_equipe_list'] = "La liste des équipes";
$langue['redirection_equipe_add'] = "L'équipe à ajouter";
$langue['redirection_equipe_edit'] = "L'équipe a été mise à jour";
$langue['redirection_equipe_dell'] = "L'équipe est bien supprimée et les membres qui y étaient sont sans équipe maintenant.";
$langue['nom_equipe'] = "Nom de l'équipe";

$langue['where_game_server'] = "Est dans l'administration des serveurs de jeux";
$langue['titre_game_server'] = "Les serveurs de jeux";
$langue['titre_game_server_gestion'] = "Gestion des serveurs";
$langue['titre_game_server_list'] = "La liste des serveurs";
$langue['redirection_game_server_add'] = "Le serveur à ajouter";
$langue['redirection_game_server_edit'] = "Le serveur a été mis à jour";
$langue['redirection_game_server_dell'] = "Le serveur est bien supprimé et les membres qui y étaient sont sans équipe maintenant.";
$langue['serveur_game_ip'] = "adresse IP de votre serveur de jeux";
$langue['serveur_game_port'] = "Query Port de votre serveur de jeux";
$langue['serveur_game_help_port'] = "<b>Pour Unreal Tournament :</b>Query Port = le port de votre serveur+1 <br /> <b>Pour Unreal Tournament 2003 :</b>Query Port = le port de votre serveur+10 <br /> <b>Pour Battlefield 1942 :</b>Pas de régle <br /> <b>Pour America\'s Army :</b>Query Port = le port de votre serveur+1<br /><br />Tout les jeux qui ne sont pas dans cette liste ont le même Query Port que le port pour s\'y connecter";
$langue['serveur_game_protocol'] = "Protocole du serveur de jeux";

$langue['where_grade'] = "Est dans l'administration des grades";
$langue['titre_grade'] = "Les grades";
$langue['titre_grade_gestion'] = "Gestion des grades";
$langue['titre_grade_list'] = "La liste des grades";
$langue['redirection_grade_add'] = "Le grade à ajouter";
$langue['redirection_grade_edit'] = "Le grade a été mis à jour";
$langue['redirection_grade_dell'] = "Le grade est bien supprimé et les membres qui y étaient sont sans grade maintenant.";
$langue['nom_grade'] = "Nom du grade";
$langue['puissance_grade'] = "Puissance du grade";
$langue['serveur_game_clan'] = "Serveur du clan";

$langue['where_liens_admin'] = "Est dans l'administration des liens";
$langue['titre_liens_admin'] = "Les liens";
$langue['titre_liens_admin_gestion'] = "Gestion des liens";
$langue['titre_liens_admin_list'] = "La liste des liens";
$langue['redirection_liens_add'] = "Le lien à ajouter";
$langue['redirection_liens_edit'] = "Le lien a été mis à jour";
$langue['redirection_liens_dell'] = "Le lien est bien supprimé";
$langue['liens_nom_site'] = "Titre du lien";
$langue['liens_url_site'] = "L'URL";
$langue['liens_image_site'] = "Bannière";
$langue['liens_test_url'] = "Tester le lien";
$langue['liens_test_no_url'] = "Pas de lien";
$langue['liens-last-repertoire'] = "Répertoires déjà entré";
$langue['liens-repertoire'] = "Répertoire";
$langue['liens-no-repertoire'] = "Sans répertoire";

$langue['where_admin_match'] = "Est dans l'administration des matchs";
$langue['titre_admin_match'] = "Les matchs";
$langue['titre_admin_match_gestion'] = "Gestion des matchs";
$langue['titre_admin_match_list'] = "La liste des matchs";
$langue['redirection_admin_match_add'] = "Le match à ajouter";
$langue['redirection_admin_match_edit'] = "Le match a été mis à jour";
$langue['redirection_admin_match_dell'] = "Le match est bien supprimé";
$langue['add_team_ok'] = "Ajouter";
$langue['add_team_reserve'] = "Réserve";
$langue['add_team_demande'] = "Demande";
$langue['match_map_liste'] = "Les maps jouées";
$langue['add_map_menu_liste'] = "Ajouer une map au menu déroulant";
$langue['add_map_liste'] = "Ajouter une map à la rotation";
$langue['dell_map_liste'] = "Supprimer la dernière map";
$langue['match_class'] = "Classification du match";
$langue['match-last-class'] = "Classe déjà entré";
$langue['match-sans-class'] = "Sans classification";

$langue['where_custom_menu'] = "Est dans l'administration du menu";
$langue['titre_custom_menu'] = "Modification du menu";
$langue['titre_custom_menu_gestion'] = "Gestion des liens";
$langue['titre_custom_menu_list_site'] = "La liste des liens du site";
$langue['titre_custom_menu_list_perso'] = "La liste des liens personalisés";
$langue['titre_custom_menu_list_module'] = "La liste des liens générés par des modules";
$langue['redirection_custom_menu_add'] = "Le lien à ajouter";
$langue['redirection_custom_menu_edit'] = "Le lien a été mis à jour";
$langue['redirection_custom_menu_dell'] = "Le lien est bien supprimé";
$langue['custom_menu_ordre'] = "Ordre d'arrivée";
$langue['custum_menu_txt'] = "Texte à afficher";
$langue['custom_menu_url'] = "Pointer sur";
$langue['custom_menu_frame'] = "Ouvrir dans une nouvelle fenêtre";
$langue['custom_menu_défiler'] = "Faire défiler le texte";
$langue['custom_menu_aide_defiler'] = "Option qui peut être désactivée sur certains thèmes, elle peut être activée mais il n\'y aura aucune différence d\'affichage";

$langue['where_module'] = "Est dans l'administration des modules";
$langue['titre_module'] = "Modification des modules";
$langue['titre_module_gestion'] = "Gestion du module";
$langue['titre_module_list'] = "La liste des modules";
$langue['redirection_module_erreur'] = "Vous devez sélectionner un module dans la liste déroulante";
$langue['redirection_module_add'] = "Le module à ajouter";
$langue['redirection_module_edit'] = "Le module a été mis à jour";
$langue['redirection_module_dell'] = "Le module est bien supprimé";
$langue['module_ordre'] = "Ordre d'arrivée";
$langue['nom_module'] = "Nom du module";
$langue['fichier_module'] = "Le module";
$langue['position_module'] = "Afficher à";
$langue['module_gauche'] = "Gauche";
$langue['module_droite'] = "Droite";
$langue['module_centre'] = "Centre";
$langue['module_etat'] = "État";
$langue['module_on'] = "Activé";
$langue['module_off'] = "Désactivé";

$langue['where_admin_mp3'] = "Est dans l'administration des MP3";
$langue['titre_admin_mp3'] = "Les MP3";
$langue['titre_admin_mp3_gestion'] = "Gestion du MP3";
$langue['titre_admin_mp3_list'] = "La liste des MP3";
$langue['redirection_admin_mp3_add'] = "Le MP3 à ajouter";
$langue['redirection_admin_mp3_edit'] = "Le MP3 a été mis à jour";
$langue['redirection_admin_mp3_dell'] = "Le MP3 est bien supprimé";
$langue['admin_mp3_source'] = "URL du MP3";
$langue['admin_mp3_autoplay'] = "Lecture automatique";
$langue['admin_mp3_shuffle'] = "Lecture aléatoire";
$langue['mp3_titre'] = "Titre";
$langue['demande_lecteur_on'] = "Cliquez ici pour écouter un MP3";

$langue['where_admin_news'] = "Est dans l'administration des nouvelles";
$langue['titre_admin_news'] = "Quoi de neuf&nbsp;?";
$langue['titre_admin_news_gestion'] = "Gestion des nouvelles";
$langue['titre_admin_news_list'] = "La liste des nouvelles";
$langue['redirection_admin_news_add'] = "La nouvelle à ajouter";
$langue['redirection_admin_news_edit'] = "La nouvelle a été mise à jour";
$langue['redirection_admin_news_dell'] = "La nouvelle est bien supprimée";
$langue['admin_news_titre'] = "Titre";
$langue['admin_news_corps'] = "La nouvelle";

$langue['where_pouvoir'] = "Est dans la gestion des pouvoirs d'un membre";
$langue['titre_pouvoir'] = "Gestion des pouvoirs de %1\$s";
$langue['pv_num_1'] = "Gestion des alertes";
$langue['pv_num_2'] = "Configuration du site";
$langue['pv_num_3'] = "Gestion des demandes de match";
$langue['pv_num_4'] = "Gestion des fichiers en téléchargement";
$langue['pv_num_5'] = "Gestion des grades";
$langue['pv_num_6'] = "Gestion de la liste de serveurs de jeux";
$langue['pv_num_7'] = "Gestion des médailles";
$langue['pv_num_8'] = "modifier le profil d'un membre (même le pouvoir et même admin)";
$langue['pv_num_9'] = "Pouvoir éditer la team dans la gestion des matchs";
$langue['pv_num_10'] = "Gestion des entraînements";
$langue['pv_num_11'] = "Gestion des équipes";
$langue['pv_num_12'] = "Supprimer une réaction à une news";
$langue['pv_num_13'] = "Gestion des liens";
$langue['pv_num_14'] = "Gestion des matchs";
$langue['pv_num_15'] = "Gestion des boutons";
$langue['pv_num_16'] = "Gestion des modules";
$langue['pv_num_17'] = "Gestion des MP3";
$langue['pv_num_18'] = "Gestion des news";
$langue['pv_num_19'] = "Gestion des rapports de match";
$langue['pv_num_20'] = "Gestion des maps du serveur de jeux";
$langue['pv_num_21'] = "Gestion des sections";
$langue['pv_num_22'] = "Supprimer un membre";
$langue['pv_num_23'] = "Gestion des smileys";
$langue['pv_num_24'] = "Envoi mail de masse";
$langue['pv_num_25'] = "Gestion du calendrier";
$langue['redirection_pouvoir_edit'] = "les pouvoirs ont été mis a jour, vous allez être redirigé sur la liste des membres";
$langue['erreur_admin_and_pouvoir'] = "Vous ne pouvez pas changer les pouvoirs d'un administrateur. Changez le niveau de cet utilisateur avant de modifier ses pouvoir";

$langue['where_admin_rapport_match'] = "Est dans l'administration des rapports de match";
$langue['titre_admin_rapport_match'] = "Les rapports de match";
$langue['titre_admin_rapport_match_gestion'] = "Gestion des rapports";
$langue['titre_admin_rapport_match_list'] = "La liste des rapports";
$langue['redirection_admin_rapport_match_add'] = "Le rapport de match est ajouter";
$langue['redirection_admin_rapport_match_edit'] = "Le rapport de match a été mis à jour";
$langue['redirection_admin_rapport_match_dell'] = "Le rapport de match est bien supprimé";
$langue['rapport_match_importation'] = "Importer un match";
$langue['rapport_match_score_final'] = "Le score final";
$langue['rapport_match_dell_importe'] = "supprimer le match importé";

// les erreurs dans les formulaire
$langue['erreur_titre'] = "Il y a une erreur dans le formulaire.";
$langue['erreur_no_group'] = "Vous n'avez pas défini un groupe pour le fichier.";
$langue['erreur_nom_vide'] = "Vous n'avez pas entré votre nom.<br />";
$langue['erreur_prenom_vide'] = "Vous n'avez pas entré votre prénom.<br />";
$langue['erreur_user_vide'] = "Vous n'avez pas entré votre login.<br />";
$langue['erreur_psw_vide'] = "Vous n'avez pas entré votre code.<br />";
$langue['erreur_user_prit'] = "Une autre personne a déjà pris ce login.<br />";
$langue['erreur_mail_vide'] = "Vous n'avez pas entré votre e-mail.<br />";
$langue['erreur_mail_prit'] = "Une autre personne a déjà pris cet e-mail.<br />";
$langue['erreur_mail_invalide'] = "Vous n'avez pas entré un e-mail valide pour votre e-mail.<br />";
$langue['erreur_msn_vide'] = "Vous n'avez pas entré votre adresse Msn Messenger.<br />";
$langue['erreur_msn_invalide'] = "Vous n'avez pas entré un e-mail valide pour votre adresse Msn Messenger.<br />";
$langue['erreur_clan_vide'] = "Vous n'avez pas entré le nom de votre clan.<br />";
$langue['erreur_jour_vide'] = "Vous n'avez pas entré le jour.<br />";
$langue['erreur_mois_vide'] = "Vous n'avez pas entré le mois.<br />";
$langue['erreur_minute_vide'] = "Vous n'avez pas entré les minutes.<br />";
$langue['erreur_heure_vide'] = "Vous n'avez pas entré les heures.<br />";
$langue['erreur_sex_vide'] = "Vous n'avez pas selectionné votre sexe.<br />";
$langue['erreur_naissance_vide'] = "Votre date de naissance est incomplète.<br />";
$langue['erreur_arme_vide'] = "Vous n'avez pas selectionné votre arme préférée.<br />";
$langue['erreur_perso_image_vide'] = "Vous n'avez pas selectionné votre image.<br />";
$langue['erreur_langue_vide'] = "Vous n'avez pas selectionné votre langue.<br />";
$langue['erreur_code_différent'] = "Vous n'avez pas tapé deux fois le même code.<br />";
$langue['erreur_time_zone'] = "Vous n'avez pas selectionné votre fuseau horaire.<br />";

$langue['where_admin_map_serveur'] = "Est dans l'administration des maps affichées sur le site";
$langue['titre_admin_map_serveur'] = "Les maps sur le serveur";
$langue['titre_admin_map_serveur_gestion'] = "Gestion des maps";
$langue['titre_admin_map_serveur_list'] = "La liste des maps";
$langue['redirection_admin_map_serveur_add'] = "La map à ajouter";
$langue['redirection_admin_map_serveur_edit'] = "La map a été mise à jour";
$langue['redirection_admin_map_serveur_dell'] = "La map est bien supprimée";
$langue['nom_map_console'] = "Nom brut";
$langue['nom_map_sortie'] = "Nom à afficher";
$langue['url_map_custom'] = "Liens pour télécharger";

$langue['where_admin_smilies'] = "Est dans l'administration des smilies";
$langue['titre_admin_smilies'] = "Les smilies";
$langue['titre_admin_smilies_gestion'] = "Gestion des smilies";
$langue['titre_admin_smilies_list'] = "La liste des smilies";
$langue['redirection_admin_smilies_add'] = "Le smilie à ajouter";
$langue['redirection_admin_smilies_edit'] = "Le smilie de match a été mis à jour";
$langue['redirection_admin_smilies_dell'] = "Le smilie de match est bien supprimé";
$langue['smilie_images'] = "Le smilies";
$langue['smilie_text'] = "Commande";
$langue['smilie_def'] = "Définition";

$langue['bt_bbcode']['b'] = 'Mettre en gras le texte. Syntaxe : [b]texte[/b]';
$langue['bt_bbcode']['i'] = 'Mettre en italique le texte. Syntaxe : [i]texte[/i]';
$langue['bt_bbcode']['u'] = 'Souligner le texte. Syntaxe : [u]texte[/u]';
$langue['bt_bbcode']['url'] = 'Insérer un lien. Syntaxe : [url=http://www.lesite.com/]nom du lien[/url]';
$langue['bt_bbcode']['liste'] = 'Liste d\'éléments. Syntaxe : [list][*]item 1[*]item 2[/list]';
$langue['bt_bbcode']['centrer'] = 'Aligne le texte. Syntaxe : [center]texte[/center]';
$langue['bt_bbcode']['size'] = 'Définir la taille du texte. Syntaxe : [size=14]texte[/size]';
$langue['bt_bbcode']['img'] = 'Mettre une image. Syntaxe : [img]http://www.lesite.com/image.png[/img]';
$langue['bt_bbcode']['video'] = 'Mettre une video. Syntaxe : [video]http://www.lesite.com/film.avi[/video]';
$langue['bt_bbcode']['swf'] = 'Mettre une animation flash. Syntaxe : [swf width=12 height=14]http://www.lesite.com/animation.swf[/swf]';

$langue['where_admin_mailiste'] = "Envoit un mail de masse";
$langue['titre_admin_mailiste'] = "Mail de masse";
$langue['redirection_admin_mailiste_add'] = "Les emails ont été envoyés";
$langue['mailiste_subject'] = "Objet";
$langue['mailiste_corps'] = "Corps";
$langue['mailiste_qui'] = "Envoyé à";
$langue['mailiste_sans_section'] = "Membre sans section";

$langue['where_admin_section'] = "Est dans l'administration des sections";
$langue['titre_admin_section'] = "Les sections";
$langue['titre_admin_section_gestion'] = "Gestion des sections";
$langue['titre_admin_section_list'] = "La liste des sections";
$langue['redirection_admin_section_add'] = "La section à ajouter";
$langue['redirection_admin_section_edit'] = "La section a été mise à jour";
$langue['redirection_admin_section_dell'] = "La section est bien supprimée, tous les membres qui y étaient se retrouvent sans section.";
$langue['admin_section_nom'] = "Nom";

$langue['admin_section_titre_limite'] = "Limité";
$langue['admin_section_limite'] = "Ne peut voir que les matchs de sa section";
$langue['admin_section_limite_true'] = "Limité";
$langue['admin_section_limite_false'] = "Peut participer à tous les matchs";
$langue['admin_section_titre_visible'] = "Visibilité";
$langue['admin_section_visible'] = "Visible dans la partie publique";
$langue['help_section'] = "<b>Explication :</b> Les sections sont utiles quand le clan à beaucoup de membres, pour faire une section A et une section B par exemple, et dans chaque section, les équipes";

// texte de formulaire
$langue['form_nom'] = "Votre nom";
$langue['form_prenom'] = "Votre prénom";
$langue['form_login'] = "Votre login";
$langue['form_psw'] = "Votre code";
$langue['form_re_psw'] = "Retapez votre code";
$langue['form_mail'] = "Votre e-mail";
$langue['form_message'] = "Votre message";
$langue['form_msn'] = "Votre adresse Msn";
$langue['form_jour'] = "Le jour";
$langue['form_mois'] = "Le mois";
$langue['form_heure'] = "L'heure";
$langue['form_min'] = "Les minutes";
$langue['form_sex'] = "Votre sexe";
$langue['form_naissance'] = "Votre date de naissance";
$langue['form_langue'] = "Votre langue";
$langue['form_web'] = "Votre site Web";
$langue['form_fv_arme'] = "Votre arme préférée";
$langue['form_slogan'] = "Votre cri de guerre";
$langue['form_image'] = "Votre image";
$langue['alt_image_profil'] = "Votre image";
$langue['form_histoire'] = "Votre histoire";
$langue['form_time_zone'] = "Votre fuseau horaire";
$langue['form_heure_ete'] = "Changez-vous d'heure en été ?";
// souvent utilisé
$langue['confirm_dell'] = "Êtes-vous certain de vouloir supprimer ceci ?";
$langue['show/hide_smilies'] = "Plus de Smilies";
$langue['alt_aide'] = "Information en plus, aide";
$langue['date_format'] = '(jj-mm-aaaa)';
$langue['sex_homme'] = "Homme";
$langue['sex_femme'] = "Femme";
$langue['date'] = "Date";
$langue['heure'] = "Heure";
$langue['détails'] = "Détails";
$langue['posteur'] = "Posteur";
$langue['nom/sex'] = "Le nom / Sexe";
$langue['nom'] = "Le nom";
$langue['action'] = "Action";
$langue['profil'] = "Profil";
$langue['ip'] = "IP";
$langue['port'] = "Port";
$langue['query_port'] = "Query Port";
$langue['protocol'] = "Protocol";
$langue['grade'] = "Grade";
$langue['role'] = "Rôle";
$langue['msn'] = "MSN M";
$langue['alt_msn'] = "MSN Messenger";
$langue['alt_mail'] = "E-mail";
$langue['alt_profil'] = "Voir son profil";
$langue['numero'] = "n°";
$langue['medailles'] = "Médailles";
$langue['avatar'] = "L'image";
$langue['pouvoirs'] = "Pouvoirs";
$langue['equipe'] = "Équipe";
$langue['section'] = "Section";
$langue['le_txt'] = "Le texte";
$langue['oui'] = "Oui";
$langue['non'] = "Non";
$langue['inconnu'] = "Indéterminé";
$langue['adr_exp'] = "Adresse expéditeur";
$langue['adr_retour'] = "Adresse de retour";
$langue['envoyer_a'] = "Envoyer à";
$langue['apercu'] = "Prévisualisation";
$langue['optionnel'] = "Optionnel";
$langue['select_time_zone'] = "GMT %d:%s heure(s)";

$langue['envoyer'] = "Envoyer";
$langue['editer'] = "Modifier";
$langue['supprimer'] = "Supprimer";
$langue['choisir'] = "Choisir";

$langue['active'] = "Activer";
$langue['desactive'] = "Désactiver";
$langue['user'] = "Membre";
$langue['admin'] = "Administrateur";

// date
$langue['lundi'] = "lundi";
$langue['mardi'] = "mardi";
$langue['mercredi'] = "mercredi";
$langue['jeudi'] = "jeudi";
$langue['vendredi'] = "vendredi";
$langue['samedi'] = "samedi";
$langue['dimanche'] = "dimanche";

$langue['janvier'] = "janvier";
$langue['fevrier'] = "février";
$langue['mars'] = "mars";
$langue['avril'] = "avril";
$langue['mai'] = "mai";
$langue['juin'] = "juin";
$langue['juillet'] = "juillet";
$langue['aout'] = "août";
$langue['septembre'] = "septembre";
$langue['octobre'] = "octobre";
$langue['novembre'] = "novembre";
$langue['décembre'] = "décembre";
?>