<?php
/****************************************************************************
 *	Fichier		: ts_webpost_central.php									*
...
if (defined('CL_AUTH'))
{
	...
	if( !empty($module_installtion))
	{
		...
		$sql = "SELECT COUNT(id) FROM ".$config['prefix']."modules WHERE call_page ='ts_webpost_central.php' OR call_page ='ts_webpost.php' LIMIT 1";
		if (! ($get = $rsql->requete_sql($sql)) )
		{
			sql_error($sql, $rsql->error, __LINE__, __FILE__);
		}
		...
		$sql = "INSERT INTO `".$config['prefix']."custom_menu` (`ordre` , `text` , `url` , `bouge` , `frame` , `module_central` , `id_module` ) VALUES ('0', '".$_POST['nom']."', 'modules/ts_webpost_central.php?from=".$id_insert."', '0', '0', '1', '".$id_insert."')";
		if (!$rsql->requete_sql($sql))
		{
			sql_error($sql, $rsql->error, __LINE__, __FILE__);
		}
...
$root_path = './../';
	...
	if ( !empty($_POST['Submit_module_webpost_centrale']) )
	{
		...
		if ($connection = fsockopen($_POST['ip'], $_POST['query_port'], $a, $b, 3))
		{
			...
			if (trim(fgets($connection, 4096)) == '[TS]')
			{
				...
				{
					redirec_text($root_path.'modules/ts_webpost_central.php?config_modul_admin=oui&id_module='.$id_module , $langue['erreur_webpost_bad_port'], 'admin');
				}
			}
		}
		else
		{
			redirec_text($root_path.'modules/ts_webpost_central.php?config_modul_admin=oui&id_module='.$id_module , $langue['erreur_webpost_no_reply'], 'admin');
		}
	}
	require($root_path.'conf/frame_admin.php');
	$template = new Template($root_path.'templates/'.$session_cl['skin']);
	$template->set_filenames( array('body' => 'modules/ts_webpost_central.tpl'));
	...
	$template->assign_block_vars('webpost_config', array(
		'ICI' => session_in_url('ts_webpost_central.php'),
		'TITRE_M' => $langue['titre_module_webpost'],
		'TITRE_AIDE' => $langue['module_webpost_titre_aide'],
		'TXT_AIDE' => sprintf($langue['module_webpost_txt_aide'], $config['site_domain'].$config['site_path'].'modules/ts_webpost_central.php'),
		'ID'=> $id_module,
		'TXT_IP' => $langue['ip'],
		'IP' => $recherche['ip'],
